...
################################################
def sobol(dim,seed): # p = dimension seed = terme (attention dim inf ou egale à 40)
...
def alternate(l_max,l_min): #liste alternée
    z = [l_max[0]]
    for i in range(len(l_min)):
        z = z + [l_min[i]] + [l_max[i+1]]
    return z

def discretize(lst): #ajouter une étape dans la discrétisation
    h=[]
...
T =  [i/25 for i in range(1,26)]
bb = generate_sobol_bridge(T,100)
